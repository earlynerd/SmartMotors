...
void SinglePairEthernet::rxCallback(void *pCBParam, uint32_t Event, void *pArg)
{
    ...
    if (userRxCallback)
    {
        if (pRxBufDesc->trxSize > SPE_FRAME_HEADER_SIZE)
         ...
         registerCallback(rxBufDesc[i].cbFunc, ADI_MAC_EVT_P1_RX_RDY);
    }
...
bool SinglePairEthernet::begin(uint8_t *mac, uint8_t cs_pin)
{
    ...
    {
        Serial.println("fail spe begin");
    }

    result = setUserContext((void *)this);
    if (result != ADI_ETH_SUCCESS)
    {
    ...
    if (result != ADI_ETH_SUCCESS)
    {
        Serial.println("fail enable defaults");
    }
...
adi_eth_Result_e SinglePairEthernet::enableDefaultBehavior()
{
    ...
    addrRule.TO_HOST = 1;
    result = addAddressFilter(macAddr, nofilt, addrRule);
    if (result != ADI_ETH_SUCCESS)
    {
        Serial.println("fail setting own address filter");
    }
    ...
    addrRule.APPLY2PORT2 = 1;
    addrRule.TO_HOST = 1;
    result = addAddressFilter(brcstMAC, nofilt, addrRule);
    if (result != ADI_ETH_SUCCESS)
    {
        Serial.println("fail setting broadcast address filter 1");
    }

    result = setCutThroughMode(false, false, true);
    if (result != ADI_ETH_SUCCESS)
    {
        Serial.println("fail setting cut through mode");
    }

    result = setPortForwardingMode(adin2111_Port_e::ADIN2111_PORT_1, true);
    if (result != ADI_ETH_SUCCESS)
    {
        Serial.println("fail setting port 1 forwarding");
    }

    result = setPortForwardingMode(adin2111_Port_e::ADIN2111_PORT_2, true);
    if (result != ADI_ETH_SUCCESS)
    {
        Serial.println("fail setting port 2 forwarding");
    }

    //result = setPromiscuousMode(adin2111_Port_e::ADIN2111_PORT_1, true);
    if (result != ADI_ETH_SUCCESS)
    {
        Serial.println("fail setting promiscuous 1");
    }

    //result = setPromiscuousMode(adin2111_Port_e::ADIN2111_PORT_2, true);
    if (result != ADI_ETH_SUCCESS)
    {
        Serial.println("fail setting promiscuous 2");
    }
    ...
    */
    ...
    if (result != ADI_ETH_SUCCESS)
    {
        Serial.println("fail Syncing Config");
    }

    result = registerCallback(linkCallback_C_Compatible, ADI_MAC_EVT_LINK_CHANGE);
    if (result != ADI_ETH_SUCCESS)
    {
        Serial.println("fail registering link callback");
    }
    ...
    for (uint32_t i = 0; i < SPE_NUM_BUFS; i++)
    {
        if (result != ADI_ETH_SUCCESS)
        {
            Serial.print("Fail submitting Rx Buffer ");
            Serial.println(i - 1);
        }
        ...
        txBufAvailable[i] = true;
        rxBufAvailable[i] = false;
        rxBufDesc[i].port = ADIN2111_PORT_1;
    ...
    //result = registerCallback(rxCallback_C_Compatible, ADI_MAC_EVT_RX_FRAME_RDY);
    if (result != ADI_ETH_SUCCESS)
    {
        Serial.println("fail Registering RX callback");
    }

    result = enable();
    if (result != ADI_ETH_SUCCESS)
    {
        Serial.println("fail enable");
    }
