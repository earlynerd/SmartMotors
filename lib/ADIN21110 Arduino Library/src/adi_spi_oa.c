...
adi_eth_Result_e oaStateMachine(adi_mac_Device_t *hDevice)
{
    ...
    switch (hDevice->state)
    {
        case ADI_MAC_STATE_CONTROL_START:
            //DEBUG_MESSAGE("oaStateMachine ADI_MAC_STATE_CONTROL_START");
            len = hDevice->cnt;
        ...
        case ADI_MAC_STATE_CONTROL_END:
            //DEBUG_MESSAGE("oaStateMachine ADI_MAC_STATE_CONTROL_END");
            ...
            if (cHdr != eHdr)
            {
                //DEBUG_MESSAGE("error cHdr %02X != eHdr %02X", cHdr, eHdr);
                hDevice->spiErr = 1;
            }
            else
            {
                //DEBUG_MESSAGE("no error cHdr != eHdr");
                hDevice->spiErr = 0;
        ...
        case ADI_MAC_STATE_DATA_START:
            //DEBUG_MESSAGE("oaStateMachine ADI_MAC_STATE_DATA_START");
        ...
        case ADI_MAC_STATE_DATA_END:
            //EBUG_MESSAGE("oaStateMachine ADI_MAC_STATE_DATA_END");
        ...
        case ADI_MAC_STATE_DATA_READ_STATUS:
            //DEBUG_MESSAGE("oaStateMachine ADI_MAC_STATE_DATA_READ_STATUS");
        ...
        case ADI_MAC_STATE_DATA_READ_PHY_REGISTER:
            //DEBUG_MESSAGE("oaStateMachine ADI_MAC_STATE_DATA_READ_PHY_REGISTER");
            /* Check header vs. echoed header for errors */
        ...
        default:
            //DEBUG_MESSAGE("oaStateMachine defualt");
            hDevice->state = ADI_MAC_STATE_READY;
            hDevice->spiState = ADI_MAC_SPI_STATE_READY;
