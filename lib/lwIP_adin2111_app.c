...
{
   LwIP_ADIN2111_t* eth = (LwIP_ADIN2111_t*) netif->state;

    adin2111_DeviceHandle_t *hDevice =   eth->hDevice;
...
static err_t LwIP_ADIN2111LinkOutput(struct netif *netif, struct pbuf *p)
{
...
static err_t LwipADIN2111Init(struct netif *netif)
{
   LwIP_ADIN2111_t* eth = (LwIP_ADIN2111_t*) netif->state;

   netif->output = etharp_output;
   netif->linkoutput = LwIP_ADIN2111LinkOutput;
...
static adi_eth_Result_e ADIN2111Init(LwIP_ADIN2111_t* eth)
{
...
//    DEBUG_RESULT("adin2111_AddAddressFilter", result, ADI_ETH_SUCCESS);
 ...
 //   result = adin2111_RegisterCallback(*hDevice, cbLinkChange, ADI_MAC_EVT_LINK_CHANGE);
...
void LwIP_Init( LwIP_ADIN2111_t* eth,  board_t *boardDetails)
{
    ADIN2111Init(eth);
    lwip_init();
    http_set_ssi_handler((tSSIHandler)ssiHandler, NULL, 0);
    httpd_init();
    if (boardDetails->ip_addr_fixed == 1)
    {
      ...
      LwipADIN2111Init, ethernet_input);
